...
{
public:
    CellTarget(const std::string& cellID = "", float targetTemp = 0.0f, int targetRPM = 0, int timestamp = 0)
        : m_cellID(cellID), m_targetTemp(targetTemp), m_targetRPM(targetRPM) , m_timestamp(timestamp){}

    // Copy constructor
    CellTarget(const CellTarget& other)
        : m_cellID(other.m_cellID), m_targetTemp(other.m_targetTemp), m_targetRPM(other.m_targetRPM), m_timestamp(other.m_timestamp) {}
