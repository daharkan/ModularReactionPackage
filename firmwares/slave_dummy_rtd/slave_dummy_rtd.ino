...
MAX31865_RTD rtd1( MAX31865_RTD::RTD_PT100, 10);
MAX31865_RTD rtd2( MAX31865_RTD::RTD_PT100, 7);
...
void setup()
{
  ...
  pinMode(7, OUTPUT);        // SPI SS (avoid becoming SPI Slave)
...
void loop()
{
  ...
  xAccel1 = rtd1.temperature();
  

  xAccel2 = rtd2.temperature();
  ...
  snprintf(outgoingMessageBefore, MESSAGE_LENGTH, "%s#%d#%s#%s#%d#%s#%s#%d", "s25_555" , 5, innerStr, extStr, rpm, ampStr, targetTempStr, targetRPM);

  Serial.println(outgoingMessageBefore);

  int checksum = calculateChecksum(outgoingMessageBefore);
  
  snprintf(outgoingMessageAfter, MESSAGE_LENGTH, "%s#%d#%s#%s#%d#%s#%s#%d*%d\n", "s25_555" , 5, innerStr, extStr, rpm, ampStr, targetTempStr, targetRPM, checksum);
