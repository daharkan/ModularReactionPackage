...
*/
  ...
  // Output control signal (e.g., to a motor)
  // Assuming output is used to control motor speed
  ...
  if (elapsedTime >= 1000) { // Update every 1 second
    ...
    // Calculate RPM
    rpm = (float)pulses * 60000 / (elapsedTime * 2); // 2 pulses per revolution (rising and falling edge)
  ...
  snprintf(outgoingMessageBefore, MESSAGE_LENGTH, "%s#%d#%s#%s#%d#%s#%s#%d", cellName , posId, innerStr, extStr, rpm, ampStr, targetTempStr, targetRPM);

  Serial.println(outgoingMessageBefore);

  int checksum = calculateChecksum(outgoingMessageBefore);
  
  snprintf(outgoingMessageAfter, MESSAGE_LENGTH, "%s#%d#%s#%s#%d#%s#%s#%d*%d\n", cellName , posId, innerStr, extStr, rpm, ampStr, targetTempStr, targetRPM, checksum);
